    ...
    <section className="w-full py-16 text-white">
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex flex-col space-y-6 mb-10 text-center">
          <h1 className="text-5xl font-extrabold text-amber-500 leading-tight">
            ENTER MCHJ
          </h1>
        </div>
        ...
        <div className="relative">
          <button
            onClick={prevSlide}
            className="absolute cursor-pointer left-0 top-1/2 -translate-y-1/2 bg-gray-800 hover:bg-gray-700 text-white px-3 py-2 rounded-full z-10"
          >
            <FaChevronLeft />
          </button>
          <button
            onClick={nextSlide}
            className="absolute cursor-pointer right-0 top-1/2 -translate-y-1/2 bg-gray-800 hover:bg-gray-700 text-white px-3 py-2 rounded-full z-10"
          >
            <FaChevronRight />
          </button>
...
const ImageCarousel = ({ images }) => {
  ...
  return (
    <div className="relative w-full h-48 bg-black">
      <img src={images[imgIndex]} alt="" className="w-full h-48" />
      {images.length > 1 && (
        <>
          <button
            onClick={prevImg}
